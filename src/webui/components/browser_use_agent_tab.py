...
from src.webui.webui_manager import ArmoGPTManager
...
def _get_config_value(
        armogpt_manager: ArmoGPTManager,
    ...
    try:
        comp = armogpt_manager.get_component_by_id(comp_id)
...
async def _handle_new_step(
        armogpt_manager: ArmoGPTManager, state: BrowserState, output: AgentOutput, step_num: int
):
    ...
    if not hasattr(armogpt_manager, "bu_chat_history"):
        logger.error(
            "Attribute 'bu_chat_history' not found in armogpt_manager! Cannot add chat message."
        )
        # Initialize it maybe? Or raise an error? For now, log and potentially skip chat update.
        armogpt_manager.bu_chat_history = []  # Initialize if missing (consider if this is the right place)
    ...
    armogpt_manager.bu_chat_history.append(chat_message)

    await asyncio.sleep(0.05)


def _handle_done(armogpt_manager: ArmoGPTManager, history: AgentHistoryList):
    ...
    armogpt_manager.bu_chat_history.append(
        {"role": "assistant", "content": final_summary}
    )


async def _ask_assistant_callback(
        armogpt_manager: ArmoGPTManager, query: str, browser_context: BrowserContext
) -> Dict[str, Any]:
    """Callback triggered by the agent's ask_for_assistant action."""
    logger.info("Agent requires assistance. Waiting for user input.")

    if not hasattr(armogpt_manager, "bu_chat_history"):
        logger.error("Chat history not found in armogpt_manager during ask_assistant!")
    ...
    # Use state stored in armogpt_manager
    armogpt_manager.bu_response_event = asyncio.Event()
    armogpt_manager.bu_user_help_response = None  # Reset previous response

    try:
        logger.info("Waiting for user response event...")
        await asyncio.wait_for(
            armogpt_manager.bu_response_event.wait(), timeout=3600.0
        )  # Long timeout
        ...
        armogpt_manager.bu_response_event = None  # Clear the event
        return {"response": "Timeout: User did not respond."}  # Inform the agent

    response = armogpt_manager.bu_user_help_response
    armogpt_manager.bu_chat_history.append(
        {"role": "user", "content": response}
    )  # Show user response in chat
    armogpt_manager.bu_response_event = (
        None  # Clear the event for the next potential request
    )
    return {"response": response}


# --- Core Agent Execution Logic --- (Needs access to armogpt_manager)


async def run_agent_task(
        armogpt_manager: ArmoGPTManager, components: Dict[gr.components.Component, Any]
    ...
    user_input_comp = armogpt_manager.get_component_by_id("browser_use_agent.user_input")
    run_button_comp = armogpt_manager.get_component_by_id("browser_use_agent.run_button")
    stop_button_comp = armogpt_manager.get_component_by_id(
        "browser_use_agent.stop_button"
    )
    pause_resume_button_comp = armogpt_manager.get_component_by_id(
        "browser_use_agent.pause_resume_button"
    )
    clear_button_comp = armogpt_manager.get_component_by_id(
        "browser_use_agent.clear_button"
    )
    chatbot_comp = armogpt_manager.get_component_by_id("browser_use_agent.chatbot")
    history_file_comp = armogpt_manager.get_component_by_id(
        "browser_use_agent.agent_history_file"
    )
    gif_comp = armogpt_manager.get_component_by_id("browser_use_agent.recording_gif")
    browser_view_comp = armogpt_manager.get_component_by_id(
    ...
    armogpt_manager.bu_chat_history.append({"role": "user", "content": task})
    ...
    yield {
        ...
        chatbot_comp: gr.update(value=armogpt_manager.bu_chat_history),
        history_file_comp: gr.update(value=None),
        gif_comp: gr.update(value=None),
    }

    # --- Agent Settings ---
    # Access settings values via components dict, getting IDs from armogpt_manager
    def get_setting(key, default=None):
        comp = armogpt_manager.id_to_component.get(f"agent_settings.{key}")
    ...
    mcp_server_config_comp = armogpt_manager.id_to_component.get(
    ...
    def get_browser_setting(key, default=None):
        comp = armogpt_manager.id_to_component.get(f"browser_settings.{key}")
        return components.get(comp, default) if comp else default
    ...
    # Pass the armogpt_manager instance to the callback when wrapping it
    async def ask_callback_wrapper(
            query: str, browser_context: BrowserContext
    ) -> Dict[str, Any]:
        return await _ask_assistant_callback(armogpt_manager, query, browser_context)

    if not armogpt_manager.bu_controller:
        armogpt_manager.bu_controller = CustomController(
            ask_assistant_callback=ask_callback_wrapper
        )
        await armogpt_manager.bu_controller.setup_mcp_client(mcp_server_config)
    ...
    try:
        ...
        if not keep_browser_open:
            if armogpt_manager.bu_browser_context:
                logger.info("Closing previous browser context.")
                await armogpt_manager.bu_browser_context.close()
                armogpt_manager.bu_browser_context = None
            if armogpt_manager.bu_browser:
                logger.info("Closing previous browser.")
                await armogpt_manager.bu_browser.close()
                armogpt_manager.bu_browser = None

        # Create Browser if needed
        if not armogpt_manager.bu_browser:
            ...
            armogpt_manager.bu_browser = CustomBrowser(
        ...
        if not armogpt_manager.bu_browser_context:
            ...
            armogpt_manager.bu_browser_context = (
                await armogpt_manager.bu_browser.new_context(config=context_config)
            )

        # --- 5. Initialize or Update Agent ---
        armogpt_manager.bu_agent_task_id = str(uuid.uuid4())  # New ID for this task run
        os.makedirs(
            os.path.join(save_agent_history_path, armogpt_manager.bu_agent_task_id),
            exist_ok=True,
        )
        history_file = os.path.join(
            save_agent_history_path,
            armogpt_manager.bu_agent_task_id,
            f"{armogpt_manager.bu_agent_task_id}.json",
        )
        gif_path = os.path.join(
            save_agent_history_path,
            armogpt_manager.bu_agent_task_id,
            f"{armogpt_manager.bu_agent_task_id}.gif",
        )

        # Pass the armogpt_manager to callbacks when wrapping them
        async def step_callback_wrapper(
                state: BrowserState, output: AgentOutput, step_num: int
        ):
            await _handle_new_step(armogpt_manager, state, output, step_num)

        def done_callback_wrapper(history: AgentHistoryList):
            _handle_done(armogpt_manager, history)

        if not armogpt_manager.bu_agent:
            logger.info(f"Initializing new agent for task: {task}")
            if not armogpt_manager.bu_browser or not armogpt_manager.bu_browser_context:
                raise ValueError(
                    "Browser or Context not initialized, cannot create agent."
                )
            armogpt_manager.bu_agent = BrowserUseAgent(
                task=task,
                llm=main_llm,
                browser=armogpt_manager.bu_browser,
                browser_context=armogpt_manager.bu_browser_context,
                controller=armogpt_manager.bu_controller,
            ...
            armogpt_manager.bu_agent.state.agent_id = armogpt_manager.bu_agent_task_id
            armogpt_manager.bu_agent.settings.generate_gif = gif_path
        else:
            armogpt_manager.bu_agent.state.agent_id = armogpt_manager.bu_agent_task_id
            armogpt_manager.bu_agent.add_new_task(task)
            armogpt_manager.bu_agent.settings.generate_gif = gif_path
            armogpt_manager.bu_agent.browser = armogpt_manager.bu_browser
            armogpt_manager.bu_agent.browser_context = armogpt_manager.bu_browser_context
            armogpt_manager.bu_agent.controller = armogpt_manager.bu_controller

        # --- 6. Run Agent Task and Stream Updates ---
        agent_run_coro = armogpt_manager.bu_agent.run(max_steps=max_steps)
        agent_task = asyncio.create_task(agent_run_coro)
        armogpt_manager.bu_current_task = agent_task  # Store the task

        last_chat_len = len(armogpt_manager.bu_chat_history)
        while not agent_task.done():
            is_paused = armogpt_manager.bu_agent.state.paused
            is_stopped = armogpt_manager.bu_agent.state.stopped
            ...
            update_dict = {}
            if armogpt_manager.bu_response_event is not None:
                update_dict = {
                ...
                await armogpt_manager.bu_response_event.wait()
            ...
            if len(armogpt_manager.bu_chat_history) > last_chat_len:
                update_dict[chatbot_comp] = gr.update(
                    value=armogpt_manager.bu_chat_history
                )
                last_chat_len = len(armogpt_manager.bu_chat_history)

            # Update Browser View
            if headless and armogpt_manager.bu_browser_context:
                try:
                    screenshot_b64 = (
                        await armogpt_manager.bu_browser_context.take_screenshot()
                    )
                    if screenshot_b64:
        ...
        # --- 7. Task Finalization ---
        armogpt_manager.bu_agent.state.paused = False
        armogpt_manager.bu_agent.state.stopped = False
        final_update = {}
        try:
            ...
            armogpt_manager.bu_agent.save_history(history_file)
            ...
            if not any(
                    ...
                    for msg in armogpt_manager.bu_chat_history
                    if msg.get("role") == "assistant"
            ):
                armogpt_manager.bu_chat_history.append(
                    {"role": "assistant", "content": "**Task Cancelled**."}
                )
            final_update[chatbot_comp] = gr.update(value=armogpt_manager.bu_chat_history)
        except Exception as e:
            ...
            armogpt_manager.bu_current_task = None  # Clear the task reference
        ...
        armogpt_manager.bu_current_task = None  # Ensure state is reset
        yield {
            user_input_comp: gr.update(
...
# --- Button Click Handlers --- (Need access to armogpt_manager)
...
async def handle_submit(
    ...
    if armogpt_manager.bu_response_event and not armogpt_manager.bu_response_event.is_set():
        logger.info(f"User submitted assistance: {user_input_value}")
        armogpt_manager.bu_user_help_response = (
            user_input_value if user_input_value else "User provided no text response."
        )
        armogpt_manager.bu_response_event.set()
        ...
        yield {
            ...
            armogpt_manager.get_component_by_id(
    ...
    # Check if a task is currently running (using _current_task)
    elif armogpt_manager.bu_current_task and not armogpt_manager.bu_current_task.done():
        logger.warning(
        ...
        async for update in run_agent_task(armogpt_manager, components):
            yield update


async def handle_stop(armogpt_manager: ArmoGPTManager):
    """Handles clicks on the 'Stop' button."""
    logger.info("Stop button clicked.")
    agent = armogpt_manager.bu_agent
    task = armogpt_manager.bu_current_task
...
async def handle_pause_resume(armogpt_manager: ArmoGPTManager):
...
async def handle_clear(armogpt_manager: ArmoGPTManager):
    ...
    armogpt_manager.bu_current_task = None

    if armogpt_manager.bu_controller:
        await armogpt_manager.bu_controller.close_mcp_client()
        armogpt_manager.bu_controller = None
    armogpt_manager.bu_agent = None

    # Reset state stored in manager
    armogpt_manager.bu_chat_history = []
    armogpt_manager.bu_response_event = None
    armogpt_manager.bu_user_help_response = None
    armogpt_manager.bu_agent_task_id = None

    logger.info("Agent state and browser resources cleared.")

    # Reset UI components
    return {
        armogpt_manager.get_component_by_id("browser_use_agent.chatbot"): gr.update(
            value=[]
        ),
        armogpt_manager.get_component_by_id("browser_use_agent.user_input"): gr.update(
            value="", placeholder="Enter your task here..."
        ),
        armogpt_manager.get_component_by_id(
            "browser_use_agent.agent_history_file"
        ): gr.update(value=None),
        armogpt_manager.get_component_by_id("browser_use_agent.recording_gif"): gr.update(
            value=None
        ),
        armogpt_manager.get_component_by_id("browser_use_agent.browser_view"): gr.update(
            value="<div style='...'>Browser Cleared</div>"
        ),
        armogpt_manager.get_component_by_id("browser_use_agent.run_button"): gr.update(
            value="▶️ Submit Task", interactive=True
        ),
        armogpt_manager.get_component_by_id("browser_use_agent.stop_button"): gr.update(
            interactive=False
        ),
        armogpt_manager.get_component_by_id(
            "browser_use_agent.pause_resume_button"
        ): gr.update(value="⏸️ Pause", interactive=False),
        armogpt_manager.get_component_by_id("browser_use_agent.clear_button"): gr.update(
            interactive=True
        ),
    }
...
def create_browser_use_agent_tab(armogpt_manager: ArmoGPTManager):
    """
    Create the run agent tab, defining UI, state, and handlers.
    """
    armogpt_manager.init_browser_use_agent()

    # --- Define UI Components ---
    tab_components = {}
    with gr.Column():
        chatbot = gr.Chatbot(
            lambda: armogpt_manager.bu_chat_history,  # Load history dynamically
    ...
    async def submit_wrapper(
        ...
        async for update in handle_submit(armogpt_manager, components_dict):
            yield update

    async def stop_wrapper() -> AsyncGenerator[Dict[Component, Any], None]:
        """Wrapper for handle_stop."""
        update_dict = await handle_stop(armogpt_manager)
        yield update_dict

    async def pause_resume_wrapper() -> AsyncGenerator[Dict[Component, Any], None]:
        """Wrapper for handle_pause_resume."""
        update_dict = await handle_pause_resume(armogpt_manager)
        yield update_dict

    async def clear_wrapper() -> AsyncGenerator[Dict[Component, Any], None]:
        """Wrapper for handle_clear."""
        update_dict = await handle_clear(armogpt_manager)
        yield update_dict
