...
async def run_deep_research(armogpt_manager: ArmoGPTManager, components: Dict[Component, Any]) -> AsyncGenerator[
    Dict[Component, Any], None
]:
    """Handles initializing and running the DeepResearchAgent."""

    # --- Get Components ---
    research_task_comp = armogpt_manager.get_component_by_id("deep_research_agent.research_task")
    resume_task_id_comp = armogpt_manager.get_component_by_id("deep_research_agent.resume_task_id")
    parallel_num_comp = armogpt_manager.get_component_by_id("deep_research_agent.parallel_num")
    save_dir_comp = armogpt_manager.get_component_by_id(
        "deep_research_agent.max_query")  # Note: component ID seems misnamed in original code
    start_button_comp = armogpt_manager.get_component_by_id("deep_research_agent.start_button")
    stop_button_comp = armogpt_manager.get_component_by_id("deep_research_agent.stop_button")
    markdown_display_comp = armogpt_manager.get_component_by_id("deep_research_agent.markdown_display")
    markdown_download_comp = armogpt_manager.get_component_by_id("deep_research_agent.markdown_download")
    mcp_server_config_comp = armogpt_manager.get_component_by_id("deep_research_agent.mcp_server_config")
    ...
    armogpt_manager.dr_save_dir = base_save_dir
    ...
    try:
        ...
        def get_setting(tab: str, key: str, default: Any = None):
            comp = armogpt_manager.id_to_component.get(f"{tab}.{key}")
        ...
        if not armogpt_manager.dr_agent:
            armogpt_manager.dr_agent = DeepResearchAgent(
                llm=llm,
        ...
        # --- 5. Start Agent Run ---
        agent_run_coro = armogpt_manager.dr_agent.run(
            topic=task_topic,
        ...
        armogpt_manager.dr_current_task = agent_task
        ...
        running_task_id = armogpt_manager.dr_agent.current_task_id
        ...
        armogpt_manager.dr_task_id = running_task_id  # Store for stop handler
        ...
        while not agent_task.done():
            ...
            agent_stopped = getattr(armogpt_manager.dr_agent, 'stopped', False)
            if agent_stopped:
        ...
        if not running_task_id and final_result_dict and 'task_id' in final_result_dict:
            ...
            armogpt_manager.dr_task_id = running_task_id
            task_specific_dir = os.path.join(base_save_dir, str(running_task_id))
            report_file_path = os.path.join(task_specific_dir, "report.md")
        ...
        # --- 8. Final UI Reset ---
        armogpt_manager.dr_current_task = None  # Clear task reference
        armogpt_manager.dr_task_id = None  # Clear running task ID
...
async def stop_deep_research(armogpt_manager: ArmoGPTManager) -> Dict[Component, Any]:
    """Handles stopping the DeepResearchAgent."""
    logger.info("Stop button clicked for deep research")

    agent = armogpt_manager.dr_agent
    task = armogpt_manager.dr_current_task
    task_id = armogpt_manager.dr_task_id
    base_save_dir = armogpt_manager.dr_save_dir
    ...
    if agent and task and not task.done():
        ...
        final_update = {
            ...
            stop_button_comp: gr.update(interactive=False),
            armogpt_manager.get_component_by_id("deep_research_agent.research_task"): gr.update(interactive=True),
            armogpt_manager.get_component_by_id("deep_research_agent.resume_task_id"): gr.update(interactive=True),
            armogpt_manager.get_component_by_id("deep_research_agent.max_iteration"): gr.update(interactive=True),
            armogpt_manager.get_component_by_id("deep_research_agent.max_query"): gr.update(interactive=True),
        }

    return final_update


async def update_mcp_server(mcp_file: str, armogpt_manager: ArmoGPTManager):
    """
    Update the MCP server.
    """
    if hasattr(armogpt_manager, "dr_agent") and armogpt_manager.dr_agent:
        logger.warning("⚠️ Close controller because mcp file has changed!")
        await armogpt_manager.dr_agent.close_mcp_client()
...
def create_deep_research_agent_tab(armogpt_manager: ArmoGPTManager):
    """
    Creates a deep research agent tab
    """
    input_components = set(armogpt_manager.get_components())
    tab_components = {}
    ...
    armogpt_manager.add_components("deep_research_agent", tab_components)
    armogpt_manager.init_deep_research_agent()

    async def update_wrapper(mcp_file):
        """Wrapper for handle_pause_resume."""
        update_dict = await update_mcp_server(mcp_file, armogpt_manager)
        yield update_dict
    ...
    all_managed_inputs = set(armogpt_manager.get_components())

    # --- Define Event Handler Wrappers ---
    async def start_wrapper(comps: Dict[Component, Any]) -> AsyncGenerator[Dict[Component, Any], None]:
        async for update in run_deep_research(armogpt_manager, comps):
            yield update

    async def stop_wrapper() -> AsyncGenerator[Dict[Component, Any], None]:
        update_dict = await stop_deep_research(armogpt_manager)
        yield update_dict
